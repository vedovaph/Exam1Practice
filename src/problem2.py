...
def problem2b(rect, n, delta, win):
    ...
    for k in range(n):
        box = rg.Rectangle(rg.Point(rect.get_upper_left_corner().x - delta*k, rect.get_upper_left_corner().y - delta*k), rg.Point(rect.get_lower_right_corner().x + delta*k, rect.get_lower_right_corner().y + delta*k))
        box.attach_to(win)
    win.render()
